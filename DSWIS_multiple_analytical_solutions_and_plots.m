...
% This script compares several (3 to 6 in the examples) failure laws
...
io       = [0 0 0 1];                % set to 1 if you want individual plots
...
t_sd     = linspace(0, pi/sqrt(A_c), 200);
...
for fl = 1:np
    ...
    p(fl) = plot(sl_v/nd_u, (mu_v-mu_d_arr(1))/(mu_s_arr(1)-mu_d_arr(1)), 'Color', cfl(fl,:), 'LineWidth', lw, 'DisplayName',lower(char(fl+64)));
    hold on;
end
p(np+1) = plot(linspace(D_s_arr(2),1.1*nd_u,10)/nd_u, -K_s*linspace(0,1.1*nd_u-D_s_arr(2),10)/Sn/(mu_s_arr(1)-mu_d_arr(1))+1, 'k--', 'LineWidth', lw, 'DisplayName', 'Loading');
set(gca,'FontSize',fs-4);
ylabel('$\tau_{nd}$', 'Interpreter', 'latex', 'FontSize', fs);
xlabel('$D_{nd}$', 'Interpreter', 'latex', 'FontSize', fs);
hl = legend('show', 'Interpreter', 'latex', 'FontSize', fs);
...
for r = 1:np
    ...
    %% Uncomment if you would like to see slopes and energies
...
%     en_s     = Sn*((mu_s-mu_i)*D_s/2+(mu_i-mu_d)*D_s)
    ...
    if isempty(idx_pc)
        idx_pc = find(vel_1c == vel_fl(end), 1, 'last');
    end
    % Coseismic to early interseismic
    idx_cei    = find(vel_1c == vel_fl(end), 1, 'last');
    ...
    legend([j(1) j(2)],'Friction','Loading', 'Interpreter', 'latex', 'FontSize', fs-4, 'location', 'southwest');
    set(gca,'FontSize',fs-4);
    ...
    grid on;
    legend(p, {'Early interseismic', 'Late interseismic', 'Preseismic', 'Coseismic'}, 'Interpreter', 'latex', 'FontSize', fs-4, 'location', 'southeast');
    set(gca,'FontSize',fs-4);
...
%     plot(time_f/nd_t, log(vel_f/nd_v), 'LineWidth', lw);
...
%         ylim([log(int_vel*0.6/nd_v) log(max(max_vco*1.8/nd_v))]);
    ...
    grid on;
    hold on;
    legend([p(2) p(3)],'Friction','Loading', 'Interpreter', 'latex', 'FontSize', fs-4, 'location', 'southwest');
    % Link axis limits
    if r == np
        subplot(2,np,1)
        yyaxis right;
