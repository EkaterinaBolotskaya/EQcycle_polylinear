...
mu_i     = 0.695;                    % initial friction coefficient
...
D_t      = 0.4835;                   % weakening 1 distance
...
st1      = pi*sqrt(D_s*M/(Sn*(mu_s-mu_i)+D_s*K_s))/40;    % time step
...
% Cannot solve for time analytically, thus, solve numerically
t_fl     = double(vpasolve(vSol == int_vel, t, [0 pi*sqrt(M/K_s)]));
...
ku_in    = -K_s*(disp_in - V_0*time_in - V_0*(t_s+t_w1+t_w2+t_fl));
tau_in   = ku_in;

time_in  = t_s + t_w1 + t_w2 + t_fl + time_in;
